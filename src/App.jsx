...
async function exportCustodyPDF({ letterheadEl, categories, totalsEl, signaturesEl, filename, pageMarginMM = 10 }) {
  ...
  const widthCandidates = [letterheadEl.offsetWidth, ...categories.map((c) => c.theadEl?.offsetWidth || 0)];
  const containerWidthPx = Math.max(...widthCandidates, 900);
  ...
  return (
    <div className="space-y-6">
      <div className="no-print">
        <Header
          title="طباعة عهدة"
          sub="زرار تنزيل PDF بيبني الملف بنفسه بترقيم صفحات مضبوط ومحدش يضيع فيه"
          action={
            <div className="flex flex-wrap gap-2">
      ...
      {custody && (
        <div id="print-area" ref={printRef} className="bg-white border rounded-2xl p-6 text-xs" style={{ borderColor: "#E3DDCE", color: "#101A2E", fontFamily: "'Cairo', sans-serif" }}>
          <div ref={letterheadRef} style={{ minWidth: 900 }}>
          ...
          <div className="totals-signatures-block" ref={totalsRef} style={{ minWidth: 900 }}>
          ...
          <div className="totals-signatures-block grid grid-cols-4 gap-6 mt-4 pt-4" ref={signaturesRef} style={{ minWidth: 900 }}>
            {SIGNATURES.map((s) => (
